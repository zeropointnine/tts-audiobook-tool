...
class AppTextUtil:
    """
    App-specific text/string util functions (parsing, processing source text, etc).
    """
    ...
    def is_ws_punc(s: str) -> bool:
        ...
        if not s:
            return True # TODO: ?...
        return all(char in AppTextUtil.ws_punc_chars for char in s)
    ...
    def split_raw_word(raw_word: str) -> tuple[str, str, str]: 
        ...
        start_idx = 0
        while start_idx < len(raw_word) and raw_word[start_idx] in AppTextUtil.ws_punc_chars:
            start_idx += 1
            
        # Find the start of trailing ws_punc_chars
        end_idx = len(raw_word) - 1
        while end_idx >= start_idx and raw_word[end_idx] in AppTextUtil.ws_punc_chars:
            end_idx -= 1
    ...
    def get_words(s: str, vocalizable_only: bool=False) -> list[str]:
        ...
        if vocalizable_only:
            return [word for word in words if not AppTextUtil.is_ws_punc(word)]
        else:
            return words

    @staticmethod
    def get_word_count(s: str, vocalizable_only: bool=False) -> int:
        return len( AppTextUtil.get_words(s, vocalizable_only=vocalizable_only) )
    ...
    def get_uncommon_words_en(raw_words: list[str]) -> list[tuple[str, int, list[str]]]:
        ...
        for raw_word in raw_words:
            word = AppTextUtil.split_raw_word(raw_word)[1] # strip outer whitespace and punctuation
        ...
        counts_dict = { k: v for k, v in counts_dict.items() if AppTextUtil.is_vocalizable(k) }
    ...
    def get_uncommon_words(raw_words: list[str]) -> list[tuple[str, int, list[str]]]:
        return AppTextUtil.get_uncommon_words_en(raw_words)
