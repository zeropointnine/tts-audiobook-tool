...
class OptionsSubmenu:
    ...
    def submenu(state: State) -> None:
        ...
        while True:
            ...
            printt(f"{make_hotkey_string('5')} Whisper transcription model {make_currently_string(state.project.stt_variant.value)}")
            printt(f"{make_hotkey_string('6')} Attempt to unload models {vram_label}")
            printt(f"{make_hotkey_string('7')} Reset contextual hints")
            printt()
