...
class TranscodeUtil:
    ...
    def ask_transcode_abr_flac_to_aac(state: State) -> None:
        ...
        if not os.path.exists(path):
            ask.ask_enter_to_continue("No such file or directory.")
            return

        if os.path.isfile(path):
            if not path.lower().endswith(".flac"):
                ask.ask_enter_to_continue("Must have \".flac\" file suffix.")
                return
            m4a_path = Path(path).with_stem(".m4a")
            if m4a_path.exists():
                ask.ask_enter_to_continue("M4A file already exists with that file stem.")
                return
            dir_flac_paths = [path]
        else:
            dir_flac_paths = []
            ...
            if not dir_flac_paths:
                ask.ask_enter_to_continue("No FLAC files in directory.")
                return
        ...
        if not flac_paths:
            ask.ask_enter_to_continue("No files to transcode.")
            return
