...
if is_first_run:
    not_oute_phrase = "when using the Oute TTS model, " if not Shared.is_oute() else ""
    printt()
    printt("This appears to be your first time running tts-audiobook-tool.")
    printt(f"As a reminder, {not_oute_phrase}you may want to adjust/experiment with the settings in the file {COL_ACCENT}config_oute.py")
