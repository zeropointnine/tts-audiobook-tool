...
class MetadataUtil {
    ...
    static async loadAppMetadata(file, url) {
        ...
        try {
            if (isFlac) {
                ...
                tagValue = await MetadataUtil.findCustomMp4Tag(file, MP4_MEAN, MP4_TAG);
            }
        } catch (error) {
     ...
     */
    static async findCustomMp4Tag(file, targetMean, targetTagName) {
     ...
     *
     */
    static async findCustomFlacTag(file, tagName) {
        ...
        while (!isLastMetadataBlock) {
            ...
            if (blockType === 4) { // VORBIS_COMMENT block
                ...
                const foundValue = MetadataUtil.parseVorbisCommentBlock(vorbisCommentDataView, tagName, textDecoder);
    ...
    static parseVorbisCommentBlock(dataView, targetTagName, textDecoder) {
