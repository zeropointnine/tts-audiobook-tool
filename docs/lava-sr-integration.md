...
- `process(sound, denoise=False)` restores one in-memory `Sound`.
- `kill()` releases the model and accelerator cache.
...
- `clear_lava_sr_upsampler()` kills and drops the adapter, then runs memory
  collection.
...
continues without upsampling. The saved preference is not rewritten. LavaSR is
not part of the startup hard-required package checks.
